...
**Design:** follow [docs/DESIGN.md](docs/DESIGN.md) — the visual system (Raycast-derived: near-black surfaces, layered shadows, radius scale; CN-KZ keeps green accent + Manrope). Pair it with the **make-interfaces-feel-better** skill in `.agents/skills/` for micro-polish (tabular-nums, scale-on-press, concentric radii, shadows-over-borders).
