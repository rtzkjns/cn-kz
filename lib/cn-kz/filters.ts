...
// ~6 самых частых типов кузова — показываем строкой без горизонтального скролла.
// (Города НЕ подсказываем: маршрут любой — Китай→СНГ, СНГ→СНГ — только полнотекстовый поиск.)
